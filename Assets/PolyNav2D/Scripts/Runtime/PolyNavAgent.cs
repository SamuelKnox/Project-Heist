...
{
    [Tooltip("This will adjust the X/Y velocity of the movement.  The pathfinding algorithm will not be affected by these values.  The Max Speed will be affected by these values.")]
    ...
    void LateUpdate()
    {
        ...
        Vector2 adjustedVelocity = new Vector2(velocity.x * VelocityMultipler.x, velocity.y * VelocityMultipler.y);

        //slow down if wall ahead
        LookAhead();

        //move the agent
        position += adjustedVelocity * Time.deltaTime;
